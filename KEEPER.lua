...
--------------------------------------------------------------------------------
function Run()
print('\27[93m>editor:\27[39m'..' '..'GrandTeam')
end
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
  ...
  if hash then
    ...
    local text = ''
    for i=1, #names do
	   if string.match(value:lower(), names[i]:lower()) and not is_mod(msg) then	     local id = msg.id_
         local msgs = {[0] = id}
...
if text:match('^leave(-100)(%d+)$') then
       ...
       reply_to(text:match('leave(.*)'),0,1,"▪ ربات به دلایلی گروه را ترک میکند\nبرای اطلاعات بیشتر میتوانید با @sudoenergy در ارتباط باشید.",1,'md')
...
--------------------------------------------------------------------------------
if text:match("^add$") or text:match("^تفعيل$") and is_sudo(msg) then
...
--------------------------------------------------------------------------------
if text:match("^rem$") or text:match("^تعطيل$") and is_sudo(msg) then
...
5 Mah ≈> 10 Toman
Na mahdod => 15 Toman
[Admin](http://telegram.me/sudoenergy)
[Report](http://telegram.me/richenergy)
[Channel](http://telegram.me/richenergy)
]]
          function inline(arg,data)
          tdcli_function({
        ID = "SendInlineQueryResultMessage",
...
--------------------------------------------------------------------------------
  if text == "الاوامر" or text == "راهنما" and is_mod(msg) then
help = [[
▫️ راهنمای ربات انرژی تی دی😐💪
➖➖➖➖➖➖➖➖➖➖➖➖➖
...
promote [usernamereplyuser-id]
🔸تنظیم ناظر

demote [usernamereplyuser-id]
🔹حذف ناظران
...
unmute [usernamereplyuser-id]
🔹حذف کاربر از لیست سایلنت ها


mute [usernamereplyuser-id]
🔸سایلنت کردن یک فرد 
...
lock 🌐 unlock  ➕ links▪️username▪️sticker▪️contact▪️english▪️persian▪️forward▪️photo▪️video▪️gif▪️music▪️audio▪️text▪️keyboard▪️tgservice▪️pin▪️join▪️selfvideo▪️spam▪️tag 🔚

🔸برای قفل کردن Lock links

🔹برای باز کردن Unlock links
از دستورات فارسی نیز میتوانید استفاده کنید
➖➖➖➖➖➖➖➖➖➖➖➖➖➖
]]
...
--------------------------------------------------------------------------------
if text:match("^iid$") or text:match("^ایدی$") and msg.reply_to_message_id == 0  then
function GetID(msg)
