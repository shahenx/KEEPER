...
elseif msg_type == 'MSG:Text' then
 --vardump(msg)
    if database:get("bot:group:link"..msg.chat_id_) == 'انتظر 🐇🎋 ' and is_mod(msg.sender_user_id_, msg.chat_id_) then if text:match("(https://telegram.me/llx8xll/%S+)") then 	 local glink = text:match("(https://telegram.me/llx8xll/%S+)") local hash = "bot:group:link"..msg.chat_id_ database:set(hash,glink) 			 send(msg.chat_id_, msg.id_, 1, 'ضع رابط جديد', 1, 'md')
      end
   end
...
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   ...
   if text:match("^تثبيت$") and is_mod(msg.sender_user_id_, msg.chat_id_) then
	   ...
	   database:set('◯↲ تم ✅ تثبيــت الرسالــه❗️'..msg.chat_id_,msg.reply_to_message_id_)
   end
   -----------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------
  if text:match("^المطور$") and is_mod(msg.sender_user_id_, msg.chat_id_) then
...
✫مــطور البـɃŌȾـوت :⇓ 
 ...
 [〘 آلقہٰٰيِٰہٰٰصۛہٰٰڕٰ_‏ᎯᏞᏫᎪᏕᏋᏒ 〙](https://telegram.me/llx8xll)
